var footerLogo = $('footer[role=contentinfo] .corporate-logo img').attr('src');

function headerLogo() {
  $('#drop-target-logo a > img').after($('#drop-target-logo a > img').clone());
  $('#drop-target-logo a > img:nth-of-type(2)').attr('src',footerLogo);
  $('#drop-target-logo a > img:first-of-type').addClass("white");
  $('#drop-target-logo a > img:nth-of-type(2)').addClass("color");
}
...
$(window).scroll(function () {
  if ($(document).scrollTop() > 100) {
      $("[role=banner]").addClass("scrolled");
      $(".navigation > ul > li > a").addClass("scrolled");
      $(".logo img").addClass("scrolled");
      $(".has-subnav > a:after").addClass("scrolled");
      $(".collapsable-btn").addClass("scrolled");
      $(".collapsable-btn .bars span").addClass("scrolled");
  } else {
      $("[role=banner]").removeClass("scrolled");
      $(".navigation > ul > li > a").removeClass("scrolled");
      $(".logo img").removeClass("scrolled");
      $(".has-subnav > a:after").removeClass("scrolled");
      $(".collapsable-btn").removeClass("scrolled");
      $(".collapsable-btn .bars span").removeClass("scrolled");
  }
}); 
...
//   if($('.photo-randomizer:has(img.u-photo)')) {
//     $('img.u-photo').addClass('photo-object-fit');
//   }
// });

$('.photo-randomizer .u-photo').objectFit({type: 'cover', hideOverflow: true});